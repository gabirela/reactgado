...
import  apiForum from "../hooks/auth";
import api from "../../gado/src/api";
import { IAuthState, IAuthContextData } from "../src/Interfaces/";
...
const AuthProvider: React.FC = ({ children }) => {
  ...
  const signIn = useCallback(
    async ({ email, password }) => {
      const response = await apiForum.authenticate({
        email,
        password,
      });
      const { token, user } = response.data;
      setAuth({ token, apiForum });
